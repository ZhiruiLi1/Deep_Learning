...
def train_vae(model, train_loader, args, is_cvae=False):
    ...
    if is_cvae == False:
      for batch in train_loader:
        x = batch[0] 
        # batch[0].shape: (128, 1, 28, 28)    inputs
        # batch[1].shape: (128)               labels for the inputs 
        # x is the inputs for the encoder 
      ...
      for batch in train_loader:
        x = batch[0]
        c = one_hot(batch[1], model.num_classes)   # using one_hot function defined above 
        with tf.GradientTape() as tape:
